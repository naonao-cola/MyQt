    ...
    : QMainWindow(parent)
{
    ...
    createMenus();
    createToolBars();
    if(showWidget->imageLabel->img.load("image.png"))
    {
        //设置图片，在image中放置图片
        showWidget->imageLabel->setPixmap(QPixmap::fromImage((showWidget->imageLabel->img)));
    }

    connect(fontComboBox,QOverload<const QString &>::of(&QComboBox::activated),this,&ImgProcessor::ShowFontComboBox);
    ...
    connect(showWidget->imageLabel,&ClickableLabel::clicked,this,[=](){
        qDebug()<<"草泥马 ";
    });
}
...
//void ImgProcessor::ChangeImg()
//{
//    QMessageBox::information(NULL,"单击","单击了我",
//                                     QMessageBox::Yes|QMessageBox::No,QMessageBox::Yes);
//    printf("图片能改变");
//    img= showWidget->imageLabel->img;
//}
...
void ImgProcessor::ShowPrintImage()
{
    ...
    if(printDialog.exec())						//(b)
    {
        ...
        QSize size = showWidget->imageLabel->img.size();				//获得图像的大小
        /* 按照图形的比例大小重新设置视图矩形区域 */
        size.scale(rect.size(),Qt::KeepAspectRatio);
        painter.setViewport(rect.x(),rect.y(),size.width(),size.height());
        painter.setWindow(showWidget->imageLabel->img.rect());          //设置QPainter窗口大小为图像的大小
        painter.drawImage(0,0,showWidget->imageLabel->img);				//打印图像
    }
}
void ImgProcessor::ShowZoomIn()
{

    if(showWidget->imageLabel->img.isNull())				//有效性判断
        return;
    QMatrix matrix;					//声明一个QMatrix类的实例
    matrix.scale(2,2);				//(a)
    showWidget->imageLabel->img = showWidget->imageLabel->img.transformed(matrix);
    //重新设置显示图形
    showWidget->imageLabel->setPixmap(QPixmap::fromImage(showWidget->imageLabel->img));
}
void ImgProcessor::ShowZoomOut()
{
    ...
    if(showWidget->imageLabel->img.isNull())
        return;
    QMatrix matrix;
...
void ImgProcessor::ShowMirrorVertical()
{
    ...
    if(showWidget->imageLabel->img.isNull())
        return;
    showWidget->imageLabel->img=showWidget->imageLabel->img.mirrored(false,true);
    showWidget->imageLabel->setPixmap(QPixmap::fromImage(showWidget->imageLabel->img));
}
...
void ImgProcessor::ShowMirrorHorizontal()
{
    ...
    if(showWidget->imageLabel->img.isNull())
        return;
    showWidget->imageLabel->img=showWidget->imageLabel->img.mirrored(true,false);
    showWidget->imageLabel->setPixmap(QPixmap::fromImage(showWidget->imageLabel->img));
}
...
{
    //设置文字颜色
    QColor color=QColorDialog::getColor(Qt::blue,this);	//(a)
    if(color.isValid())
    {
        QTextCharFormat fmt;
